...
| 无 DADAO-11 引用 | ❌ 失败 | ras_of/ras_uf 引用了 DADAO-11-AEE |
...
| ras_of/ras_uf 检查 | ras_of, ras_uf | ⚠️ 引用 DADAO-11-AEE，需改为 SimRISC-00 |
...
**注**：前一审查记录中提到 "fpexcp: 浮点异常" 规则，但实际 legality_rules.yaml 中不存在该规则（grep 确认 0 处 fpexcp 引用）。无 FPEXCP 相关规则是正确的。
...
**问题 1（阻断）**：`legality_rules.yaml` 中 `ras_of` 和 `ras_uf` 两条规则的 `spec_cite` 和 `wiki_cite` 引用了 `DADAO-11-AEE §返回地址栈`，违反了"所有引用应指向 SimRISC-00/01/02/03/04（不应引用 DADAO-11）"的要求。

- `ras_of`（第 219-225 行）：`spec_cite: "DADAO-11-AEE §返回地址栈"`, `wiki_cite: "DADAO-11-AEE §返回地址栈 (L204)"`
- `ras_uf`（第 227-235 行）：`spec_cite: "DADAO-11-AEE §返回地址栈"`, `wiki_cite: "DADAO-11-AEE §返回地址栈 (L216)"`

**修正建议**：将这两条规则的引用改为 `SimRISC-00 §压栈流程` 和 `SimRISC-00 §弹栈流程`（spec.md 中 RAS 内容已正确引用 `[SimRISC-00 §压栈流程]` 和 `[SimRISC-00 §弹栈流程]`）。
