...
DADAO 提供 4 组用户可见寄存器，每组 64 个，每个寄存器 64 位（机器字长）。[DADAO-11 §寄存器]
...
- `rb0[63:48]` 恒为 0 [DADAO-11 §基址寄存器]
- 硬件复位后 rb0 初值为 `cfx_power_hypv_excp_vector` [DADAO-11 §基址寄存器]

#### §1.3.3 rf0

`rf0` 为浮点状态寄存器（FCSR）。[DADAO-11 §浮点状态寄存器]
...
浮点指令执行后，若 rf0[4:0] 中任意异常状态位变为 1，硬件触发 FPEXCP 异常。FPEXCP 可通过 excp_cause_mask 屏蔽。被屏蔽时，结果寄存器按 IEEE 754 写入正常计算结果，设置 pending 位，mask 解除后触发异常；未被屏蔽时，硬件保证无副作用——目标寄存器不更新，PC 停留在触发指令（精确异常）。[DADAO-11 §浮点状态寄存器]

#### §1.3.4 ra0–ra63

返回地址栈（Return Address Stack），后入先出。[DADAO-11 §返回地址栈]
...
- ra1–ra63 构成 RegRAS，ra63 为栈顶 [DADAO-11 §返回地址栈]
- ra0 低 48 位为 0 时仅 RegRAS，最多 63 个返回地址；非 0 时存在 MemRAS [DADAO-11 §返回地址栈]
- 异常进入和退出不改变 ra0–ra63 的内容 [DADAO-11 §返回地址栈]

### §1.4 数据表示

基础数据类型：[DADAO-11 §数据表示]
...
浮点格式符合 IEEE 754 标准。[DADAO-11 §原始数据类型]

### §1.5 存储模型

- 64 位地址空间，有效虚拟地址为 48 位 [DADAO-11 §存储模型]
- 高 16 位（bits[63:48]）在地址计算时被硬件忽略 [DADAO-11 §存储模型]
...
以 ra63 为 RegRAS 栈顶。[DADAO-11 §压栈流程]
...
| FPEXCP | 浮点指令执行后 rf0[4:0] 任意异常位变为 1 | [DADAO-11 §浮点状态寄存器] |
| RASOF | RegRAS 压栈溢出（调用深度超过 63） | [DADAO-11 §压栈流程] |
| RASUF | RegRAS 弹栈下溢（栈空时 ret） | [DADAO-11 §弹栈流程] |
