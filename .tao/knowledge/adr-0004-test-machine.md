...
|------------|------|----------|-----------|------------------|
| boot ROM `0xffff_ffff_0000`–`0xffff_ffff_ffff`（64 KiB，只读） | 允许（`PC[1:0]≠0` → IALIGN `0x84`） | 允许（任意宽度） | **ILLI `0x82`**（只读区域） | **MALIGN `0x81`** |
| Exit port `0xffff_8000_0000`–`0xffff_8000_0007`（8 B，只写） | 不允许 → `0x8F` | **ILLI `0x82`**（只写区域） | 仅 8 B 对齐 `st.o` → 正常退出（D3）；非 8 B `st.b/w/t` 或 `stm.*` → **ILLI `0x82`** | **MALIGN `0x81`** |
| RAM `0xffff_0000_0000`–`0xffff_00ff_ffff`（16 MiB，读写） | 允许（`PC[1:0]≠0` → IALIGN `0x84`） | 允许（任意宽度） | 允许（任意宽度） | **MALIGN `0x81`** |
...
;    set.zw rb16, wp3, 0xffff：写 wyde3（bits[63:48]）=0xffff，其余 48 位清 0 → 0xffff_0000_0000
;    or.w  rb16, wp2, 0x8000：wyde2（bits[47:32]）|= 0x8000 → 0xffff_8000_0000
set.zw  rb16, wp3, 0xffff      ; rb16 = 0xffff_0000_0000
or.w    rb16, wp2, 0x8000      ; rb16 = 0xffff_8000_0000
...
st.o    rd16, rb16, 0          ; mem64[0xffff_8000_0000] = 0 → host $? = 0x00
...
```asm
; 1. 构造 exit port 地址 rb16 = 0xffff_8000_0000
set.zw  rb16, wp3, 0xffff
or.w    rb16, wp2, 0x8000      ; rb16 = 0xffff_8000_0000

; 2. 构造 RAM 基址 rb17 = 0xffff_0000_0000
set.zw  rb17, wp3, 0xffff      ; rb17 = 0xffff_0000_0000

; 3. 触发 MALIGN：8B load 到 0xffff_0000_0001（非 8B 对齐）
ld.o    rd16, rb17, 1          ; 有效地址 = 0xffff_0000_0000 + 1 = 0xffff_0000_0001 → MALIGN → host $? = 0x81
...
;    set.zw rb1, wp3, 0xffff → 0xffff_0000_0000；or.w rb1, wp2, 0x00ff → 0xffff_00ff_0000
set.zw  rb1, wp3, 0xffff       ; rb1 = 0xffff_0000_0000
or.w    rb1, wp2, 0x00ff       ; rb1 = 0xffff_00ff_0000（RAM 顶部 0xffff_00ff_ffff 附近）

; 2. 构造 RAM 入口地址 rb2 = 0xffff_0000_0000
set.zw  rb2, wp3, 0xffff       ; rb2 = 0xffff_0000_0000
...
- **地址构造**：新地址高 wyde 全为 `0xffff`，示例用 `set.zw rbha, wp3, 0xffff` + `or.w rbha, wpN, immu16` 逐 wyde 构造。
