...
| M1 实现 | 🔄 进行中（spec ✅ M1 完成；infra：`002t`~`009t`、`013t` 已验证（`013t` = 按原始仓库名命名；里程碑顺延为 `014m`）；`010t`~`012t` 待开始；testcases：`001k`~`009t` 已验证（15 个 `isa/*.yaml`、597 条；154 数据级缺口待补）；llvm：`002t`~`003t` 已验证（`003t` = Triple 注册 + 最小 target 骨架：ADR-0007 Accepted + `build-mc` 真实构建 + `llvm-mc --version` 含 `dadao - DADAO SimRISC` + 最小 lit PASS；F3/F4 deferred）；qemu/integ 待推进） |
