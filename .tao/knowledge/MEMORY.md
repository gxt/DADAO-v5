...
| M1 实现 | 🔄 进行中（spec ✅ M1 完成；infra：`002t`~`009t`、`013t` 已验证（`013t` = 按原始仓库名命名；里程碑顺延为 `014m`）；`010t`~`012t` 待开始；testcases：`001k`~`009t` 已验证（15 个 `isa/*.yaml`、597 条；154 数据级缺口待补）；llvm：`002t`~`004t` 已验证（`004t` = Register TableGen：256 RegisterDef + 6 RegisterClass + CodeGen 可编译 + `build-mc` 门禁含 `LLVMDADAOCodeGen`）；qemu/integ 待推进） |
