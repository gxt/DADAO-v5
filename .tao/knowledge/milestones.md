...
- `spec`：`SPEC-002t` → `003t` → `SPEC-004t` → {`005t`、`006t`} → `007t` → `008t`（`002t`/`003t` 待重新生成）
...
**第 4 层 — 集成验证**（依赖第 3 层）
- `verif`：`VERIF-002t` → `003t` → `004t` → {`006t`、`008t` → `009t`}；`005t`/`007t`/`011t` 可并行；`010t` 待 `LLVM-012m`+`QEMU-014m`
