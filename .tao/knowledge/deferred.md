...
- **数据已随上一版 `002t` 一并丢弃（陈旧引用修正）**：仓库内**无任何 `tests/vectors/isa/*.yaml`**（`git ls-files tests/` = 0）。`003t`~`007t` 的向量数据须**从零生成**（依据 `002t` 的 `schema.md` + `contracts/opcodes.yaml` + `contract-isa.md`/ADR-0004）；`001k`/`003t`~`007t`/`009t`/`010m` 任务书中的「重组旧文件 / 修复现有向量」引用已同步删除（旧→新映射保留为历史说明）。**（2026-09-16 更新：`003t` 已完成 6 个 `reg-*.yaml`；`004t`~`007t` 仍须从零生成。）**
