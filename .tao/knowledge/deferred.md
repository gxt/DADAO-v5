...
- **`set.ow-rd` 多 wyde 位置未覆盖（`003t` 遗留）**：`003t` 的 `set.ow-rd` semantic 仅覆盖 `wp0`；多 wyde 位置（`wp1`/`wp2`/`wp3`）的边界测试由 `009t` 全量再审计兜底（`009t` 任务范围已含 wyde-position（`rwii`）重推导审计，此处显式登记以便追溯）。
