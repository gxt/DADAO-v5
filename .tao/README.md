...
- **过渡期任务**：不属于单一里程碑、而是「**修正上一阶段 + 为下一阶段铺路**」的任务，`**项目里程碑**` 记为 **`M<i>→M<i+1>`**（如 `M1→M2`）。此类任务**只能**是 `t`，不产生 `k`/`m`，不破坏 `k↔m` 一一对应；**不进入** `milestones.md` 的任何模块列（既不计入 M1 也不计入 M2）；出口判据为达到 `已验证`——「修正上一阶段」整体结束的判据是**无未终态的 `M1→M2` 任务**（用户裁定 2026-09-25：里程碑只是大任务的标志，不是推进的阻碍，故不为此新建里程碑）。已置 `里程碑` 的模块 `m` **不因过渡任务重开**，按 `AGENTS.md`「增加专门任务」分支处理
