...
| `LLVM-012m` | LLVM MC 里程碑 | 里程碑标记 | `LLVM-002t`~`LLVM-011t` |

- **依赖关系**：`002t → 003t → 004t → 005t → 006t → 007t → 008t → 009t → 010t`；`002t` 依赖 infra 的 `INFRA-006t`（Makefile 编排），`003t` 依赖 `SPEC-007t`（ELF 合约），`004t` 依赖 `SPEC-004t`（ABI 合约），`010t` 依赖 `SPEC-006t`（Test Machine ADR）；`011t`（RA 指令）依赖 `009t` 与 spec；`012m` 汇总全部。
