...
| `TESTCASES-010t` | 数据级覆盖率缺口消解（第一批） | `generate_isa_vectors.py` + reg-arith/logic/shift-extend/compute | 114 缺口（112 legality + 2 boundary）：全补 active case | `009t` | 待开始 |
| `TESTCASES-011t` | 数据级覆盖率缺口消解（第二批） | `generate_isa_vectors.py`/`generate_ctrl_br.py`/`generate_ctrl_jump_call_ret.py` + reg-cond-assign/imm-block/ctrl-br/ctrl-jump + `validate_vectors.py` | 35 缺口（19 legality + 10 boundary + 6 overlap）+ validator 修复 + 门控转严 | `010t` | 待开始 |
| `TESTCASES-012m` | 里程碑 | — | 核验门槛含 F1/F5/F6/F7/F10 + 149 缺口消解（gap=0） | 全部 | 待开始 |
...
| ~~`TESTCASES-011t`~~ | ~~保留编码 UNDI 向量表达~~（**编号撤销**） | — | 改由 `008t` 承载（原编号排在 `012m` 之后，违反 `nnn` 递增） | — | 撤销 |
...
- **依赖关系与下发建议**：
  ...
  - `002t → 003t → 004t → 005t → 006t → 007t → 008t → 009t → 010t → 011t → 012m` 为**推荐默认全串行**：`inventory.md`（含 `file` 列）与 `schema.md`/`validate_vectors.py` 为多任务共享，串行最稳。
