...
- **【前置阻断·009t 登记 → 010t+011t 消解】**：`009t` 发现数据级覆盖率缺口（149 条，经 validator 修正：原 154 减 5 条 `deferred C-27` 不计缺口），由 `TESTCASES-010t`（114 条：reg-arith/logic/shift-extend/compare）与 `TESTCASES-011t`（35 条：reg-cond-assign/imm-block + ctrl-br/ctrl-jump + boundary + overlap + validator 门控转严）分两批消解。**两任务均完成且 `validate_vectors.py` gap = 0 后，本里程碑方可置 `里程碑`**。详见各任务书与 `.tao/knowledge/deferred.md`。
