...
- `tools/testcases/validate_vectors.py`：覆盖率标记逻辑重构，`covered_*` 的填充只发生在精确匹配分支，且仅 1 处调用
...
5. **grep 验证**：`grep -n "covered.*add" tools/testcases/validate_vectors.py` 应只有 1 处。
...
- 本项目：`tools/testcases/validate_vectors.py`、`contracts/opcodes.yaml`、`tests/vectors/isa/`
