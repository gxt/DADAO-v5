# VERIF-008t: 分支语义 harness 扩展

**模块**：verif
**项目里程碑**：M1
**依赖**：`VERIF-004t`、`TESTSUITE-008t`
**状态**：待开始
...
- 输入：
  - `VERIF-004t` 的 `tests/scripts/build_test_binary.py`、`tests/scripts/run_qemu_test.py`
  - `tests/vectors/isa/control-flow.yaml`（`TESTSUITE-008t` 写入的 deferred semantic 桩）
  - `.tao/knowledge/contract-isa.md` §5（条件跳转/无条件跳转偏移语义）
  - `verif/opcodes.yaml`（分支/跳转 0.5.3 编码与格式）
- 输出：
  - `tests/scripts/build_test_binary.py`：新增 `build_branch_test_binary()` 与 `branch_behavior` 调度
  - `tests/vectors/isa/control-flow.yaml`：激活 ≥16 条 branch/jump semantic 测试（由本任务与 `TESTSUITE` 协同，数据修改以 `TESTSUITE` 归属为准）
- 约束：
...
2. 激活 `control-flow.yaml` 的 branch/jump semantic 测试（`TESTSUITE-008t` 已标记为 deferred 的桩）。
...
**offset 字段**：从 `contract-isa.md` §5 与 `verif/opcodes.yaml` 的格式字段手推（PC-relative 单位/基准须以 v5 合约与 `TESTSUITE-008t` 结论为准）。

**覆盖范围**（每条 taken + not-taken）：`br.n`/`br.nn`/`br.z`/`br.nz`/`br.p`/`br.np`（riii，单寄存器）+ `br.eq`/`br.ne`（rrii，双寄存器），以及 `jump-iiii`/`jump-rrii`（无条件）。call/ret 由 `VERIF-009t` 处理。
...
- `tests/vectors/isa/control-flow.yaml`：激活 ≥16 条 branch/jump semantic 测试（与 `TESTSUITE` 协同）
- 完成区附激活前后 PASS/FAIL 与条数

## 与 DADAO-0628 的差异（0.4.1 → 0.5.3）

1. **助记符**：`brn/brnn/brz/brnz/brp/brnp` → `br.n/br.nn/br.z/br.nz/br.p/br.np`；`breq/brne` → `br.eq/br.ne`；`unimp` → `illi`。另有 0.5.3 新增 `br.z-rb`/`br.nz-rb`（本任务按 `TESTSUITE-008t` 范围决定是否覆盖）。
2. **跳转偏移语义**：v5 `contract-isa.md` §5 为 `PC = rb0 + (imm << 2)`；分支/跳转/调用均相对 `rb0`。`rb0` 语义（当前 vs 下一条）以 v5 合约与 `TESTSUITE-008t` 结论为准，**不得沿用 0.4.1 的 `pc_next` 假设**。
3. **branch target 公式修复属实现层**：0.4.1 在 `translate.c` 修 `pc_next→pc_next+4`；v5 该修复属 `qemu` 模块（`QEMU-012t`），本任务只保证向量与 harness 正确。
4. **编码来源**：0.4.1 yaml 里手写 `0x2A04XXXX` 等；v5 分支编码从 `verif/opcodes.yaml` + 手推 offset 得出，不复制 0.4.1 字节。
5. **向量归属**：semantic 测试数据在 `TESTSUITE-008t` 桩基础上激活；本任务以 harness 扩展为主，数据修改须与 `TESTSUITE` 边界一致。
...
6. **call/ret 不在本任务**：`DL-029a` 明确 deferred 到 `DL-030a`（v5 对应 `VERIF-009t`）。
...
- 本项目：`.tao/knowledge/contract-isa.md` §5、`verif/opcodes.yaml`、`tests/vectors/isa/control-flow.yaml`
