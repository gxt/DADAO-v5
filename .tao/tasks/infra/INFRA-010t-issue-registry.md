...
| llvm（ISS-038~047, 070~073） | 14 | LLVM-013m 已置里程碑 ✓ | 均不阻断 |
...
**③ M1-gate 分类审查**：
- 逐条核对 6 个 M1 里程碑状态：SPEC-011m ✓、TESTCASES-012m ✓、LLVM-013m ✓、INFRA-014m（任务依赖）、QEMU-021m（待开始，被 fence 阻断）、INTEG-004m（被 QEMU-021m 阻塞）
...
**里程碑状态（实读 `**状态**` 行）**：`SPEC-011m`=里程碑、`TESTCASES-012m`=里程碑、`LLVM-013m`=里程碑、`QEMU-021m`=待开始、`INFRA-014m`=待开始、`INTEG-004m`=待开始。
...
| llvm（ISS-038–047,070–073） | 14 | `LLVM-013m` 已置里程碑 | 否 |
