...
- **差异**：
  ...
  - 测试机权威不同：exit port 协议、内存图、MALIGN/ILLI/UNDI 可观测行为以 v5 `.tao/knowledge/adr-0004-test-machine.md`（`SPEC-008t` 产出）为准，不照抄 0628 `docs/adr/0004-test-machine.md`。
...
| `QEMU-003t` | Target Skeleton | `components/qemu/patches/0001-dadao-target-skeleton.patch`、`series`、最小冒烟 | `QEMU-002t`、`SPEC-008t` |
...
| `QEMU-008t` | 控制流 + RB 指令 | `components/qemu/patches/0005-dadao-ctrl-flow.patch`、向量补充 | `QEMU-007t`、`SPEC-008t` |
...
- **依赖关系**：`002t → 003t → 004t → 005t → 006t → 007t → 008t → 009t → 010t → 011t → 012t`；`002t` 依赖 infra 的 `INFRA-006t`（Makefile 编排）；`003t` 依赖 `SPEC-008t`（Test Machine ADR，提供内存图/复位值/exit 协议）；`004t` 依赖 `SPEC-003t`（编码表）与 `VERIF-002t`（合法性规则）；`008t` 依赖 `SPEC-008t`（fault/exit 可观测）；`012t` 依赖 `TESTSUITE-008t`（控制流向量）；`013m` 汇总全部。
...
- M1 范围为标量核心（`contract-isa.md` §3 标量整数、§4 地址/内存中 RD/RB 部分、§5 控制流、§7 系统指令中测试机所需 `swym`/`illi`/`fence`）；浮点 §6 与 LR/SC/特权指令按 M1 范围排除（保留 UNDI/ILLI 桩）。
