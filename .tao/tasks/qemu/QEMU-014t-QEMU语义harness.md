...
**模块**：qemu
**项目里程碑**：M1
**依赖**：`QEMU-013t`、`TESTCASES-003t`、`SPEC-006t`
**状态**：待开始
...
- 输入：
  ...
  - `QEMU-013t` 交付的 `qemu-system-dadao`（构建路径以 `INFRA-006t` Makefile 为准）
...
3. **halt 语义**：0.5.3 的退出/停机指令与 `halt` 助记符按 `contract-isa.md` §7.5 与 `contracts/opcodes.yaml` 确定；exit port 写协议以 v5 ADR 为准。
