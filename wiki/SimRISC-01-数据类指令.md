...
逻辑运算指令需要两个源操作数，一个目的操作数。MISC-byte/wyde/tetra/octa 子表中的 `and`/`or`/`xor`/`xnor`（后缀 `.b`/`.w`/`.t`/`.o`）提供四种固定位宽的逻辑运算。操作数格式为 `orrr`。
...
`xnor` 指令为逻辑同或运算，运算规则：相同为一，相异为零。与异或运算规则相反。即两个操作数值相同时结果为1，两个操作数不一样时结果为0。
...
汇编器应提供 `not.b`/`not.w`/`not.t`/`not.o` 伪指令，用于对操作数进行指定位宽的按位取反。伪指令必须指定数据宽度后缀，汇编器根据后缀展开为对应位宽的 `xnor` 指令。
