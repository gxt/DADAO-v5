...
操作数类型 `brrr` 的 `add`/`sub` 提供带bit position的加减运算。`bpN` 指定有效位数，仅 bits[N:0] 参与运算，目的寄存器高位符号扩展。若结果溢出（超出 bits[N:0] 表示范围），溢出部分静默丢弃。`rdhb` 不能为 `rd0`，否则触发 ILLI 异常。

```simrisc
add     bpN, rdhb, rdhc, rdhd
sub     bpN, rdhb, rdhc, rdhd
```
...
另一种操作数类型为 `brrr`，增加bit position参数 `bpN`（N=0..63），表示 bits[N:0] 参与比较。源操作数按 bpN 截断后比较，结果（-1/0/1）写入目的寄存器 bits[N:0]，高位符号扩展。`bp63` 表示全 64 位。`rdhb` 不能为 `rd0`，否则触发 ILLI 异常。

```simrisc
cmps    bpN, rdhb, rdhc, rdhd
cmpu    bpN, rdhb, rdhc, rdhd
```

例如 `cmps bp7, rd1, rd2, rd3` 将 rd2 和 rd3 的低 8 位按有符号比较，结果写入 rd1 的低 8 位，rd1[63:8] 按符号扩展。
...
操作数类型 `brrr` 的 `muls`/`mulu` 提供带bit position的乘法运算。`bpN` 指定有效位数，源操作数只取 bits[N:0]，结果仅保留 bits[N:0]，高位按有符号（muls，符号扩展）或无符号（mulu，零扩展）填充。`rdhb` 不能为 `rd0`，否则触发 ILLI 异常。

```simrisc
muls    bpN, rdhb, rdhc, rdhd
mulu    bpN, rdhb, rdhc, rdhd
```

操作数类型 `brrr` 的 `divs`/`divu` 提供带bit position的除法运算。`bpN` 指定有效位宽，源操作数只取 bits[N:0]，结果仅保留 bits[N:0] 的商，高位符号扩展（divs）或零扩展（divu），余数被抛弃。`rdhb` 不能为 `rd0`，否则触发 ILLI 异常。

```simrisc
divs    bpN, rdhb, rdhc, rdhd
divu    bpN, rdhb, rdhc, rdhd
```

操作数类型 `brrr` 的 `rems`/`remu` 提供只返回余数的除法。`bpN` 指定有效位宽，源操作数只取 bits[N:0]，结果仅保留 bits[N:0] 的余数，高位符号扩展（rems）或零扩展（remu），商被抛弃。`rdhb` 不能为 `rd0`，否则触发 ILLI 异常。

```simrisc
rems    bpN, rdhb, rdhc, rdhd
remu    bpN, rdhb, rdhc, rdhd
```
...
```simrisc
and     bpN, rdhb, rdhc, rdhd
orr     bpN, rdhb, rdhc, rdhd
xor     bpN, rdhb, rdhc, rdhd
xnor    bpN, rdhb, rdhc, rdhd
```
...
以上四条逻辑运算指令的 bits[63:N+1] 保持目的寄存器原有值不变。
...
shlu是逻辑左移，shru是逻辑右移，shrs是算数右移。操作数类型为 `brrr` 和 `brri`。移位量（shamt）取 `rdhd` 的低 6 位（寄存器形式）或 `immu6` 的低 6 位（立即数形式）。
...
exts是符号扩展，extz是零扩展。操作数类型为 `brrr` 和 `brri`。
...
rdhb[63:N+1] = rdhb[63:N+1]                              // 高位不变
```
`bp63` 为全 64 位。`hd ≤ bpN`，否则 ILLI。

```simrisc
shlu    bpN, rdhb, rdhc, rdhd
shru    bpN, rdhb, rdhc, rdhd
shrs    bpN, rdhb, rdhc, rdhd

exts    bpN, rdhb, rdhc, rdhd       ; rdhd = 源数据最高位，须 <= bpN
extz    bpN, rdhb, rdhc, rdhd

shlu    bpN, rdhb, rdhc, immu6
shru    bpN, rdhb, rdhc, immu6
shrs    bpN, rdhb, rdhc, immu6

exts    bpN, rdhb, rdhc, immu6       ; immu6 = 源数据最高位，须 <= bpN
extz    bpN, rdhb, rdhc, immu6
```
