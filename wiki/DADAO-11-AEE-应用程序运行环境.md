...
> **版本：0.9.2**（与 ABI 版本号一致，同步更新。基于 SimRISC 0.4.1 指令系统设计）
...
从内存加载时，`ldb`/`ldbu`（8-bit）、`ldw`/`ldwu`（16-bit）、`ldt`/`ldtu`（32-bit）分别完成符号扩展或零扩展至 64-bit。存储时，`stb`/`stw`/`stt` 截取低 N 位写入内存。

**加减法**：结果产生 128-bit（rdha = 高 64 位，rdhb = 低 64 位），低 N 位天然正确。若不需要高 64 位，可将 `rdha` 设为 `rd0` 丢弃。

```simrisc
; 有符号 32-bit 加法
ldt     rd2, rbsp, x_offset          ; 符号扩展加载
ldt     rd3, rbsp, y_offset
...
```simrisc
; 有符号 32-bit 乘法
ldt     rd2, rbsp, x_offset
ldt     rd3, rbsp, y_offset
muls    rd0, rd4, rd2, rd3           ; rd0丢弃高64位，rd4=低64位
stt     rd4, rbsp, z_offset

; 有符号 32-bit 除法（rdha = 余数，rdhb = 商）
ldt     rd2, rbsp, x_offset
ldt     rd3, rbsp, y_offset
divs    rd0, rd4, rd2, rd3           ; rd0丢弃余数，rd4=商
stt     rd4, rbsp, z_offset
```
...
- 加减法结果为 128-bit（rdha:rdhb），若 `rdha` ≠ 0（符号扩展位之外出现了非零值），则低 64-bit 结果溢出。`rdha` 为 `rd0` 时丢弃溢出信息。

```simrisc
; 有符号 32-bit 加法，检测溢出
add     rd2, rd3, rd4, rd5          ; rd2:rd3 = rd4 + rd5
; 若 ldt 符号扩展正确，rd2 ≠ 0 即表示 32-bit 溢出
brnz    rd2, overflow_handler
```
...
- `rb0` 固定为当前指令的下一条指令的绝对地址（48 位有效），只读，写无效。`rb0[63:48]` 恒为 0。硬件复位后 `rb0` 初值为 `cfx_power_hypv_excp_vector`（见 SEE §2.1）。
...
浮点指令执行后，若 rf0[4:0] 中任意异常状态位变为 1，硬件触发 FPEXCP 异常。FPEXCP 可通过 excp_cause_mask 屏蔽。被屏蔽时，结果寄存器按 IEEE 754 标准写入默认值（qNaN/sNaN），不触发异常；未被屏蔽时，硬件保证无副作用——目标寄存器不更新，PC 停留在触发指令（精确异常）。
