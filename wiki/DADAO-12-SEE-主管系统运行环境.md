...
> **版本：0.7.1**（与 SBI 版本号一致，同步更新。基于 SimRISC 0.5.0 指令系统设计）
...
每个核芯功能扩展中针对用户模式（user）有以下11个寄存器（cg=0）：
...
每个核芯功能扩展中针对监狱模式（jail）有以下11个寄存器（cg=1）：
...
每个核芯功能扩展中针对主管模式（supv）有以下11个寄存器（cg=2）：
...
| cg | rc | 寄存器名 | regname | 初始值 | 访问 | 说明 |
|----|----|---------|---------|--------|------|------|
| 10 | 0 | timer pending | cfx_timer_pending | 0 | RW | 待处理定时器中断源，写0清位 |
...
| cg | rc | 寄存器名 | regname | 初始值 | 访问 | 说明 |
|----|----|---------|---------|--------|------|------|
| 8 | 0 | uart pending | cfx_uart_pending | 0 | RW | 待处理UART中断源，写0清位 |
...
| cg | rc | 寄存器名 | regname | 初始值 | 访问 | 说明 |
|----|----|---------|---------|--------|------|------|
| 8 | 0 | power pending | cfx_power_pending | 0 | RW | 待处理电源管理中断源，写0清位 |
...
**精确异常**：所有同步异常和异步中断均为精确异常——PC 指向触发指令（同步）或下一条指令边界（异步），指令未完成，目标寄存器/内存/RA 未更新（无副作用）。页表步进异常（GPTRAP/SPTRAP/PFTRAP/NxPERM）同为精确——访存未执行。IALIGN 为精确异常——取指前检测 PC[1:0] ≠ 00，PC 停留在非法地址。FPEXCP 被屏蔽时不触发异常，结果寄存器按 IEEE 754 标准写入默认值；未被屏蔽时与其他同步异常一致，硬件保证无副作用。
...
1. **确定核芯功能扩展**：
   ...
   - 其它指令执行触发的 ILLI/UNDI/RASOF/RASUP/FPEXCP/IALIGN/MALIGN 异常：
...
5. **判断异常原因是否被屏蔽**：检查 `cfx_⟨cfxname⟩_<mode>_excp_cause_mask` 对应位，若为1则该异常原因被屏蔽
    - 同步异常中仅 FPEXCP 可被屏蔽——被屏蔽时目的寄存器已按 IEEE 754 写入正常结果，只需设置 `cfx_⟨cfxname⟩_pending` 对应位（与异步中断一致），后续 mask 解除时触发。其余同步异常均为不可屏蔽，不存在此路径
    - 异步中断若被屏蔽，则处于待处理状态（pending），并将对应位置1到 `cfx_⟨cfxname⟩_pending` 寄存器中（OR语义，不影响其他pending位）。软件可通过写0清除对应pending位。
...
if (cfx_⟨cfxname⟩_excp_cause_nonmaskable & cause) == 0:
    ...
    if temp_cfx_code != inner_cfx_code
       and (inner_cfx_mask & (1 << temp_cfx_code)) != 0:
        ...
        else:     cfx_⟨cfxname⟩_pending ← cfx_⟨cfxname⟩_pending | cause
        return
    ...
    if (cfx_⟨cfxname⟩_<mode>_excp_cause_mask & cause) != 0:
        cfx_⟨cfxname⟩_pending ← cfx_⟨cfxname⟩_pending | cause   // 同步可屏蔽或异步：设置 pending
        return
