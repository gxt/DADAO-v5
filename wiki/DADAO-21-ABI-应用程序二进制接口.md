...
> **版本：0.9.2**（与 AEE 版本号一致，同步更新。基于 SimRISC 0.5.1 指令系统设计）
...
`not`伪指令，等价于 `xnor bp63, rdhb, rdhc, rd0` 或 `xnor bp63, rdhb, rd0, rdhc`（源和目的可同可异，将 `rd0` 作为 `rdhd` 操作数实现对另一操作数的 64 位取反）。详细规则见 SimRISC-01 § 逻辑运算。
...
cfx_tlb_inv_loop:
    ; 仅 invalid PTBR 0-15
    shlu    bp63, rd3, rd2, 42              ; rd3 = PTBR 索引 << 42（构造 addr_start，VA[47:42]=PTBR 编号）
