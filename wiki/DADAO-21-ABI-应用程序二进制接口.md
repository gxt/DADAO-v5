...
> **版本：0.9.2**（与 AEE 版本号一致，同步更新。基于 SimRISC 0.5.2 指令系统设计）
...
- **`rbfp` 以上**（高地址）：调用者帧，存放寄存器溢出参数区和 varargs 保存区。两者均在调用点由调用者写入，按参数声明顺序从左到右、8 字节对齐、连续紧凑存放。溢出区发生在某 bank 寄存器槽位用完后，而 varargs 保存区则无论是否使用寄存器都将所有参数写入栈上。
- **`rbfp` 以下**（低地址）：当前帧，存放被调用者保存的寄存器、局部变量等，向下延伸至 `rbsp`。
