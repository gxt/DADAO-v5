...
- `brrr`：四个操作数，第一个为位掩码 `bm0–bm63`（在 `ha[5:0]`），后三个为 rd 寄存器（hb=目的，hc/hd=源）。`bmN` 表示 bits[N:0] 参与运算。目的寄存器高位符号扩展。`bm63` 为全 64 位
- `brri`：三个操作数，第一个为位掩码 `bm0–bm63`（在 `ha[5:0]`），一个 rd 寄存器（hb=目的，hc=源），一个 6 位立即数在 `hd[5:0]`
...
| 0000-1xxx     | exts-brri     | extz-brri     | add-brrr      | sub-brrr      | muls-brrr     | mulu-brrr     |               |               |
...
| 000-xxx   | swym-oiii |           |           |           |           |               |               |               |
| 001-xxx   |           |           |           |           |           |               |               |               |
| 010-xxx   |           | shlu-orrr | shrs-orrr | shru-orrr |           |               |               |               |
| 011-xxx   |           | shlu-orri | shrs-orri | shru-orri |           |               |               |               |
| 100-xxx   |           |           |           |           |           |               |               |               |
