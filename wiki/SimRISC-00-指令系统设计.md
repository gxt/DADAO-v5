...
- `oiii`：minor-opcode + 18 位立即数
- `orii`：minor-opcode + 一个寄存器 + 12 位立即数
- `orri`：minor-opcode + 两个寄存器 + 6 位立即数在 `hd[5:0]`
- `orrr`：minor-opcode + 三个寄存器

以下两种格式含 `b`（bitmask，6 位位掩码编码），bitmask 始终在 `ha[5:0]`。`bmN` 表示 bits[N:0] 为有效位宽，高位行为由各指令定义，`bm63` 为全 64 位：

- `brrr`：bitmask + 三个 rd 寄存器（hb=目的，hc/hd=源）
- `brri`：bitmask + 两个 rd 寄存器（hb=目的，hc=源）+ 一个 6 位立即数在 `hd[5:0]`
