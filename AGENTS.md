...
- **参考目录**：`DADAO-0628` 和 `DADAO` 作为工程经验参考，不复制其代码。两者 commit 已锁定在 `manifests/`，只读工作树检出在 **`.dadao/<id>/`**（`make fetch-refs` 幂等重建）。
...
- **留证须捕获被检命令自身退出码**：不得用 `cmd | tee log; echo $?`——管道后 `$?` 是**管道末端 `tee`** 的退出码（恒 0），会把失败记成通过；须用 `cmd > log 2>&1; rc=$?; … ; echo "EXIT=$rc"`，或 `cmd | tee log; echo "EXIT=${PIPESTATUS[0]}"`，或 `set -o pipefail`。教训：`INTEG-005t` 的改前基线日志 `INTEG-005t-check_pre.log` 末行记 `EXIT=0`（真实为 **1**），证据文件与完成区结论自相矛盾，由 reviewer 独立重跑才确认真实退出码。
...
- **反例注入须可复原**：注入反例后必须**确认已还原**（`git status`/`git diff` 核对，或改在临时树操作），**且还原必须包含重建**（源码还原 ≠ 二进制还原），并在完成区给出还原证据；**不得在污染的工作区/二进制上得出验收结论**（教训：`QEMU-008t` 还原命令路径写错，工作区残留 `add.so-rb` 截断，致「38/38」不可复现、实为 37/38；`QEMU-011t` 只还原源码未重建，探针跑在注入过的二进制上，误把 005t 探针失败当成真实回归）。
- **探针的寄存器/内存值构造须回读校验**：写操作数/期望值时不得假定某条指令能装下任意位宽（如 `add.si` 只有 18 位立即数），须用正确序列（如 `set.zw wp3` + `or.w`）并在关键用例中**回读校验**。教训：`QEMU-011t` 的 `_set_rd_to_val` 用 `set.zw(0)+add_si(64位值)` 致被除数静默变 0，误报 4 条失败。
- **注入有效性**：注入后须验证 `git diff --name-only` **非空**，确保注入确实改动了目标文件——防止「空注入」导致「注入后仍全绿」的假结论（教训：`QEMU-008t` 第 3 轮 sed 模式写错、源码未变，探针仍报 38/38）。
- **探针分支须双向验证**：手写探针的 `br_nz`/`br_ne` 等条件分支，必须（i）**全文件逐条**核对「目标 = 分支指令 + 偏移」是否指向预期的 FAIL 分支（不得抽样），（ii）给出「仅令该断言比较值错 → FAIL」的真实输出。教训：本模块已 **3 次**因分支偏移/极性错误导致断言恒真（`QEMU-008t` 两支写同一结果、`QEMU-010t` `cmp_uo` 误用致恒等、`QEMU-010t` `br_nz(18,4)` 指向 PASS 分支）。
